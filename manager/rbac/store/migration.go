...
type StoreManagerMigrateFromMajor0Minor6ToMajor0Minor7 struct {
	DB           *sqlx.DB
	StoreManager *StoreManager
}

func (s *StoreManagerMigrateFromMajor0Minor6ToMajor0Minor7) GetManager() ladon.Manager {
	return s.StoreManager
}

// Get retrieves a policy.
func (s *StoreManagerMigrateFromMajor0Minor6ToMajor0Minor7) Migrate() error {
	...
	for _, p := range pols {
		...
		if err := s.StoreManager.Create(p); err != nil {
...
func (s *StoreManagerMigrateFromMajor0Minor6ToMajor0Minor7) Create(policy Policy) (err error) {
	conditions := []byte("{}")
